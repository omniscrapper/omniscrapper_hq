...
Hanami.configure do
  ...
  environment :production do
    logger level: ENV.fetch('LOG_LEVEL', :info), formatter: :json, filter: []

    mailer do
      delivery :smtp, address: ENV.fetch('SMTP_HOST'), port: ENV.fetch('SMTP_PORT') if ENV.has_key?('SMTP_HOST')
    end
  end
end
